 ...
 */
async function inlineAsset(absoluteUrl, thresholdBytes = INLINE_ASSET_THRESHOLD_BYTES) {
 ...
 */
async function fetchStylesheet(absoluteUrl) {
  try {
    const controller = new AbortController();
 ...
 */
function serializeAttributes(element, baseUrl, styleClassName, pendingAssets) {
 ...
 */
function traverseNode(node, baseUrl, styleMap, counter, pendingAssets, depth = 0) {
 ...
 */
function assembleCssFromStyleMap(styleMap) {
  const rules = [];
 ...
 */
function assembleHTML(bodyHtml, styleMap, metadata, inlinedStylesheets = []) {
  const {
 ...
 */
async function serializePageToHTML({
  inlineImages = true,
